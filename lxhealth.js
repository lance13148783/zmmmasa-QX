...
^https:\/\/sports\.lifesense\.com\/sport_service\/sport\/sport\/uploadMobileStepV2 url script-request-body lxhealth.js
...
function hackingRequestBody(data) {
    ...
    const initSteps = 99100,
        notifyContents = `原始数据为：${steps}`;
    if (~~steps < initSteps) {
        steps = initSteps + Math.ceil(Math.random() * 900);
